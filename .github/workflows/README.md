...
| --- | --- | --- |
| [`ci.yml`](ci.yml) | push to `main`, every PR, manual | Tests on Node 22 and 24, repository hygiene, documentation links, shell scripts, commit messages, workflow syntax |
...
Every check in `ci.yml` has a local equivalent, so nothing is CI-only:

```bash
node --test              # the whole suite, no install needed
