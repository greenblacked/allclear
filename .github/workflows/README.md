...
| [`release.yml`](release.yml) | push of a `vMAJOR.MINOR.PATCH` tag | Checks the tag matches `package.json` and is on `main`, reruns typecheck, tests and build, then publishes a GitHub Release with that version's `CHANGELOG.md` section as notes |
