...
| --- | --- | --- |
| [`ci.yml`](ci.yml) | push to `main` or `dev`, PRs into either, manual | Typecheck, tests, build and SSR smoke on the Node version pinned in `.nvmrc` and on Node 24; repository hygiene, documentation links, shell scripts, commit messages, workflow syntax |
...
| [`pr-title.yml`](pr-title.yml) | PRs into `main` or `dev`, including title edits | The PR title is a Conventional Commit. A squash merge makes it the commit that `release.yml` reads once it reaches `main`. The branch name follows [CONTRIBUTING.md#branches](../../CONTRIBUTING.md#branches) |
